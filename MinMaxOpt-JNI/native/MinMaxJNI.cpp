...
extern "C" JNIEXPORT jdoubleArray JNICALL Java_MinMaxJNI_findMinMax(JNIEnv* env, jobject obj, jstring jDirectory) {
    ...
    double elapsed = std::chrono::duration<double>(end - start).count(); // Now using double

    // Change return type to jdoubleArray
    jdoubleArray result = env->NewDoubleArray(3);
    if (result == nullptr) return nullptr;

    jdouble values[3] = {static_cast<jdouble>(globalMin.load()),
                         static_cast<jdouble>(globalMax.load()),
                         elapsed}; // Store execution time as double
